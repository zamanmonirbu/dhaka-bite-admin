...
export function ServiceHeader() {
  ...
  const handleSaveMenu = (menuData: any) => {
    ...
    // Here you would typically save the menu data to your backend
    ...
    <>
      <header className="flex h-16 items-center justify-between border-b border-[#b0ceb1]/20 bg-white px-6">
        <div className="flex items-center gap-4">
          <h1 className="text-2xl font-semibold text-[#1d1f2c]">Service</h1>
        </div>

        <div className="flex items-center gap-4">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#777980]" />
            <Input placeholder="Search menus..." className="w-80 pl-10 border-[#b0ceb1] focus:border-[#016102]" />
          </div>

          <Button onClick={handleAddMenu} className="bg-[#016102] hover:bg-[#016102]/90 text-white">
      ...
      <AddMenuModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSaveMenu} />
    </>
  )
}
